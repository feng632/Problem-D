...
## 6. 06 模型评价的回填(论文手/建模手)

06 节不新建表/图,数值全部复用 04 已回填的表,另有若干**定性判断**需依据结果写结论:

| 06 落点 | 待填内容 | 来源(数值复制即可) |
|---|---|---|
| 6.1.1 τ*、$F_1$、AUC | 数值 | 表 `tab:q1-cls` |
| 6.1.1 mAP@0.5 / mAP@0.5:0.95 | 数值 | 表 `tab:q2-det` |
| 6.1.1 Dice / mIoU | 数值 | 表 `tab:q2-seg` |
| 6.1.1 $AP_s$ 累计提升 | 数值 | 表 `tab:q3-ablation`(末行 − 首行) |
| 6.1.1 mAP@0.5 最大衰减 | 数值 | 表 `tab:q3-robust`(各扰动行取最大) |
| 6.1.1 FPS、是否满足实时性 | 数值+判断 | 表 `tab:q3-eff` |
| 6.1.2 AP 相对大小是否与先验一致 | 定性判断 | 表 `tab:q2-det`(Hole 应最低) |
| 6.1.2 混淆矩阵是否集中“深凹痕↔破洞” | 定性判断 | 图 `fig:q2-confusion` |
| 6.1.2 聚焦残损而非背景 | 定性判断 | 图 `fig:q3-gradcam`、`fig:q2-visual` |
| 6.2 $F_1(\tau)$ 在 τ* 附近平坦/敏感 | 定性判断 | 图 `fig:q1-metrics` |
| 6.2 τ_nms 区间、分辨率 mAP 增益/FPS 下降 | 数值+判断 | 图 `fig:q3-sens` |
| 6.2 top-4 哪种更优及差距 | 数值+判断 | 表 `tab:q3-top4` |

注意:06 无需新脚本、无需新增表/图;回填顺序应为**先完成 04 各表回填 → 再填 06**(06 依赖 04 的结果)。

> 07 模型推广已由建模手写就,纯定性、无待回填数值,无需同步回填点。
